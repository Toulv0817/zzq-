 ...
 * @date 2022/8/30
 */
public class ZxScListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    List<ScZxListBean.ResultBean> list;
    Context context;

    public ZxScListAdapter(List<ScZxListBean.ResultBean> list, Context context) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof ViewHodel){
            ...
            h.name.setText(list.get(position).getId()+"");
            h.phone.setText(list.get(position).getCreateTime()+"");
...
//            });
    ...
    private ZxListAdapter.dj laaa;

    public void setLaaa(ZxListAdapter.dj laaa) {
        this.laaa = laaa;
    }
    //接口回调
